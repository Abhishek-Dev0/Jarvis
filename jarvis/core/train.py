...
def main():
    ...
    if args.resume:
        ...
        if start_step >= tcfg.max_steps:
            raise SystemExit(
                f"--max-steps {tcfg.max_steps} is <= the checkpoint's step {start_step - 1}. "
                f"The training loop would run 0 iterations and 'done' would be a lie. "
                f"Pass --max-steps > {start_step} (e.g. --max-steps {start_step + 1000}) "
                f"to actually fine-tune further.")
    else:
